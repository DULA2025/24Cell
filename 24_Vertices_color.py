...
prime_vertices = [p - 1 for p in prime_candidates[:24]]  # Reduced to 24 vertices
...
# 3D Visualization with labels and color coding
plotter = pv.Plotter()

# Create vertex positions using a modified sphere layout
# Using golden spiral distribution for better spacing
...
for i in range(n):
    ...
    # Adjusted radius for better spacing with fewer vertices
    r = 5
...
for i in range(num_vertices):
    for j in range(i + 1, num_vertices):
        if adjacency_matrix[i, j]:
            ...
            plotter.add_mesh(line, color='black', opacity=0.3, line_width=2)
